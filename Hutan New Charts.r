...
suppressPackageStartupMessages(library(sqldf)) #Load package to run SQL commands inside of R
suppressPackageStartupMessages(library(ggplot2)) #Load graphing cpabilities
...
g1 <- qplot(stars, data = yelp_tbl, geom = "bar", fill = I("red"), color = I("black"), 
            ...
            xlab = "Stars", ylab = "Count")
g1 <- g1 + theme(panel.border = element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank())   
